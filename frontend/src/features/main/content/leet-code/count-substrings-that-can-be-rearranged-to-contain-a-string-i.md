...
You are given two strings `word1` and `word2`. Return the number of substrings of `word1` such that the characters of the substring can be rearranged (permuted) to contain `word2` as a substring, i.e., the substring's character counts are greater than or equal to `word2`'s character counts for every letter.
...
Use a sliding window with two pointers. Maintain the character-frequency requirements of `word2` and a running count of how many distinct required characters are currently satisfied (`window count >= need count`) as the right pointer advances. For each right endpoint, advance the left pointer as far as possible while the window still satisfies all requirements; once it no longer does, the last position where it was satisfied determines how many valid starting indices exist for the current right endpoint (all indices from `0` up to `left - 1`). Sum these counts as the right pointer scans the string.
